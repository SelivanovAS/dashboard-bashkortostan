...
`scheduled(event, env)` ([worker.js:2846](../../cloudflare-worker/worker.js#L2846)):
...
Маршрутизатор — `fetch(request, env)` ([2889](../../cloudflare-worker/worker.js#L2889)).
...
| `/admin/visits` | GET | `handleAdminVisits` ([2764](../../cloudflare-worker/worker.js#L2764)) | `OWNER_SECRET` | Сводка посещений одним KV-list: дни, итоги, список устройств. |
...
- `vid` — случайный UUID, который фронт создаёт при первом визите и хранит в
  `lsKey('visit_id')` (`getVisitId`, [4897](../../app.js#L4897)). ⚠️ Обязательно
